...
void GuiTextBox(Rectangle rec, char* buffer, int bufferSize, int ID, string placeholder) {
    ...
    // Click to activate
    if (hover && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) activeBoxID = ID;
    
    // Draw Box
    DrawRectangleRec(rec, RAYWHITE);
    if (activeBoxID == ID) DrawRectangleLines(rec.x, rec.y, rec.width, rec.height, RED);
    else DrawRectangleLines(rec.x, rec.y, rec.width, rec.height, DARKGRAY);
        ...
        switch (currentScreen) {
            ...
            case SCREEN_LOGIN:{
                ...
                if (GuiButton((Rectangle{350, 350, 140, 40}), "Login")) {
                    ...
                    for (int i = 0; i < userCount; i++) {
                        if (users[i].email == em && users[i].password == pa) {
                            ...
                            if (users[i].role == "owner") currentScreen = SCREEN_OWNER_DASH;
                            else currentScreen = SCREEN_CUSTOMER_DASH;
                    ...
                    if(!found) activeBoxID = -1; // Simple way to trigger "Invalid" text
                }
                ...
                if (GuiButton((Rectangle{510, 350, 140, 40}), "Register")) {
                    ...
                    // Clear buffers
                    memset(nameBuffer, 0, 64); memset(emailBuffer, 0, 64); 
                    memset(passBuffer, 0, 64); memset(roleBuffer, 0, 64);
                }

                if (activeBoxID == -1) DrawText("Invalid Credentials!", 400, 400, 20, RED);
            }
                break;
            ...
            case SCREEN_REGISTER:{
                ...
                // Cancel button
                if (GuiButton((Rectangle{10, 50, 80, 30}), "Back")) currentScreen = SCREEN_LOGIN;
            }
                break;
            ...
            case SCREEN_OWNER_DASH:{
                ...
                if (GuiButton(Rectangle{300, 400, 400, 60}, "Manage Menus / Add Items")) {
            ...
            case SCREEN_CREATE_RESTAURANT:{
                ...
                if (GuiButton((Rectangle{10, 50, 80, 30}), "Back")) currentScreen = SCREEN_OWNER_DASH;
            }
                break;

            // ---------------------------------------------------------
            // CASE 7: OWNER VIEW ORDERS
            // ---------------------------------------------------------
            ...
            case SCREEN_OWNER_ADD_MENU_ITEM: {
                ...
                for(int i=0; i<restaurantCount; i++) { 
                    if(restaurants[i].id == selectedRestaurantID) rName = restaurants[i].name; 
                }
                ...
                if (GuiButton(Rectangle{350, 300, 300, 50}, "Add Item to Menu")) {
                    ...
                    try {
                        ...
                        m.price = 0.0; // Default if they typed text instead of numbers
                    }
                ...
                for(int i=0; i<menuItemCount; i++) {
                    if(menuItems[i].restaurantID == selectedRestaurantID) {
                        ...
                        displayY += 25;
                        if(displayY > 650) break; // Stop drawing if we run off screen
                    }
                }

                // Back Button
                if (GuiButton(Rectangle{10, 50, 80, 30}, "Back")) currentScreen = SCREEN_OWNER_VIEW_RESTAURANTS;
            ...
            case SCREEN_CUSTOMER_ORDERS: {
                ...
                if (GuiButton(Rectangle{10, 50, 80, 30}, "Back")) currentScreen = SCREEN_CUSTOMER_DASH;
                ...
                // 1. SHOW CURRENT ORDERS (Placed, Preparing, In Delivery)
                for (int i = 0; i < orderCount; i++) {
                    if (orders[i].customerID == currentUserID && 
                       (orders[i].status != "Delivered" && orders[i].status != "Rejected")) {
                        ...
                        // Status Color
                        Color statusColor = ORANGE; // Default for "Placed"
                        if(orders[i].status == "Preparing") statusColor = BLUE;
                        if(orders[i].status == "in delivery") statusColor = PURPLE;
                        
                        DrawText(("Status: " + orders[i].status).c_str(), 60, yPos + 55, 18, statusColor);
                ...
                for (int i = 0; i < orderCount; i++) {
                    if (orders[i].customerID == currentUserID && 
                       (orders[i].status == "Delivered" || orders[i].status == "Rejected")) {
                        ...
                        Color statusColor = (orders[i].status == "Delivered") ? GREEN : RED;
